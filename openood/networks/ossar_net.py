...
class GradCAM_Attn(nn.Module):

    def __init__(self, channels, reduction):
        super(GradCAM_Attn, self).__init__()
    ...
    def forward(self, x):
        ...
        spatial_mask = torch.sum(spatial_mask,1).reshape(size[0],size[2]*size[3])
        spatial_mask = self.sigmoid(spatial_mask)
        spatial_mask = spatial_mask.reshape(size[0],1,size[2],size[3])
...
class OSSARLayer(nn.Module):
    def __init__(self, feat_dim=2, num_classes=10, weight_pl=0.1, temp=1.0):
        super(OSSARLayer, self).__init__()
        self.weight_pl = weight_pl
        self.temp = temp
        self.Dist = Dist(num_classes, feat_dim=feat_dim) # num_class = 15
    ...
    def forward(self, x, labels=None):
        dist_aug_p = self.Dist(x, center=self.points, metric='cos')
        logits = dist_aug_p
        ...
        loss_m = F.relu(diff-margin).mean()
        loss = loss + 0.05 * loss_r + 0.05 * loss_m
